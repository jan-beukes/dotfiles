...
syntax match odinFloat "\v<\d%(_?\d)*%(\.\.@!)?%(\d%(_?\d)*)?%([eE][+-]?\d%(_?\d)*)?" display
...
" Stolen from the go syntax
syntax match odinImaginaryInteger "\<-\=\%(0\|\%(\d\|\d_\d\)\+\)i\>" display
syntax match odinImaginaryFloat "\v<\d%(_?\d)*%(\.\.@!)?%(\d%(_?\d)*)?%([eE][+-]?\d%(_?\d)*)?i" display
...
highlight def link odinImaginaryInteger Number
highlight def link odinFloat Float
highlight def link odinImaginaryFloat Float
